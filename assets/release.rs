...
directories = "6"
open = "5"
reqwest = { version = "0.13", default-features = false, features = ["charset", "gzip", "hickory-dns", "http2", "rustls", "system-proxy", "zstd"] }
...
async fn main() -> Result<(), Error> {
    ...
    Command::new("cargo").arg("+stable").arg("install-update").arg("--git").arg("systray-wurstmineberg-status").check("cargo install-update").await?;
